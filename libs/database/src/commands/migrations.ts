...
export class DbOperationsCommand {
  ...
  async migrateStatus(_cli: ConsoleIO): Promise<void> {
    ...
    const [completed, pending]: Record<string, any>[][] = await knex.migrate.list(
      this.migratorConfig
    );
    const statusList = [];
  ...
  async migrationUp(_cli: ConsoleIO): Promise<void> {
    const knex = ObjectionService.connection();
    const [batch, migrations]: [number, string[]] = await knex.migrate.latest(this.migratorConfig);
  ...
  async migrateReset(_cli: ConsoleIO) {
    ...
    const [, migrations]: [number, string[]] = await knex.migrate.down(this.migratorConfig);
